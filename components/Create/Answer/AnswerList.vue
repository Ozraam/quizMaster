...
<template>
    <h3>
        Answers
        <button aria-label="Add answer" class="add-answer-button" @click="$emit('add-answer')">
            +
        </button>
    </h3>
    <ul class="answer-list">
        <CreateAnswer v-for="(answer, index) in answers" :key="answer.id" :question-index="questionIndex" :answer="answer"
